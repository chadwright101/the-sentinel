import Link from "next/link";
import Image from "next/image";

import classNames from "classnames";

import { PostProps } from "../../../_types/post-types";

interface HomePageCategoryLatestGridProps {
  posts: PostProps[];
  hoveredIndex: number | null;
  setHoveredIndex: (index: number | null) => void;
  categorySlug?: string;
}

const HomePageCategoryLatestGrid = ({
  posts,
  hoveredIndex,
  setHoveredIndex,
  categorySlug,
}: HomePageCategoryLatestGridProps) => {
  const truncateText = (text: string, maxLength: number) => {
    const cleanText = text.replace(/<[^>]*>/g, "");
    return cleanText.length > maxLength
      ? cleanText.substring(0, maxLength) + "..."
      : cleanText;
  };

  const isLatestNews = !categorySlug;
  const isNewsCategory = categorySlug === "general-news";
  const isSportCategory = categorySlug === "sport";
  const isEntertainmentCategory = categorySlug === "entertainment";
  const isLifestyleCategory = categorySlug === "lifestyle";
  const isRealEstateCategory = categorySlug === "real-estate";

  return (
    <div className="grid grid-cols-1 gap-5 tablet:grid-cols-2 desktop:gap-[50px] desktop:grid-cols-3">
      {posts.map((post, index) => {
        const isFirstPost = index === 0;

        return (
          <article
            key={post.id}
            className={classNames("desktop:grid", {
              "desktop:row-span-2 desktop:gap-[50px]":
                isFirstPost && !isLatestNews,
              "desktop:order-last": !isFirstPost,
              "grid-cols-2 desktop:col-span-2":
                isFirstPost &&
                !(
                  isEntertainmentCategory ||
                  isRealEstateCategory ||
                  isLatestNews
                ),
              "grid-cols-3 desktop:col-span-3":
                isFirstPost &&
                (isEntertainmentCategory || isRealEstateCategory),
              "desktop:col-start-2 desktop:row-start-1":
                index === 1 && isLatestNews,
            })}
          >
            <Link
              href={`/posts/${post.slug}`}
              className={classNames("grid gap-4 mb-2", {
                "grid-cols-[1fr_1.75fr] tablet:grid-cols-1": !isFirstPost,
                "desktop:h-full": isFirstPost,
                "col-span-2": isEntertainmentCategory || isRealEstateCategory,
              })}
              onMouseEnter={() => setHoveredIndex(index)}
              onMouseLeave={() => setHoveredIndex(null)}
            >
              <div className="overflow-hidden">
                <Image
                  src={post.jetpack_featured_media_url}
                  alt={post.title.rendered}
                  width={800}
                  height={600}
                  sizes="(max-width:600px) 100vw, (max-width:1100px) 50vw, 500px"
                  className={classNames("object-cover", {
                    "aspect-[5/3]": isFirstPost,
                    "aspect-[1.2/1] tablet:aspect-[5/3]": !isFirstPost,
                    "desktop:hover:scale-[102%] ease-in-out duration-300 delay-75":
                      hoveredIndex === index,
                    "desktop:h-full desktop:min-h-[600px]":
                      isFirstPost &&
                      !isLatestNews &&
                      !isLifestyleCategory &&
                      !isRealEstateCategory,
                    "desktop:aspect-[3/3.25]":
                      isFirstPost && isLifestyleCategory,
                  })}
                />
              </div>
              <h3
                className={classNames(
                  "font-inter font-bold text-18px desktop:text-20px ease-in-out duration-300 delay-75",
                  {
                    "place-self-center justify-self-start": !isFirstPost,
                    "desktop:hover:opacity-90": hoveredIndex === index,
                    "desktop:hidden": isFirstPost && !isLatestNews,
                  }
                )}
              >
                {post.title.rendered}
              </h3>
              <p
                className={classNames(
                  "text-12px font-normal font-newsreader tablet:text-16px text-black",
                  {
                    hidden: !isFirstPost,
                    "block tablet:hidden": isFirstPost,
                  }
                )}
              >
                {truncateText(post.excerpt.rendered, 150)}
              </p>
            </Link>
            <div className="flex flex-col gap-4">
              <h3
                className={classNames(
                  "hidden font-inter font-bold text-18px desktop:text-20px ease-in-out duration-300 delay-75",
                  {
                    "place-self-center justify-self-start": !isFirstPost,
                    "desktop:hover:opacity-90": hoveredIndex === index,
                    "desktop:block": isFirstPost && !isLatestNews,
                  }
                )}
              >
                {post.title.rendered}
              </h3>
              <p
                className={classNames(
                  "hidden text-12px font-normal font-newsreader tablet:text-16px text-black",
                  {
                    hidden: !isFirstPost,
                    "block tablet:hidden": isFirstPost,
                    "desktop:block": isFirstPost && !isLatestNews,
                  }
                )}
              >
                {truncateText(
                  post.excerpt.rendered,
                  isLifestyleCategory || isRealEstateCategory ? 300 : 500
                )}
              </p>
              <Link
                href={`/posts/${post.slug}`}
                className={classNames(
                  "text-blue p-2 -m-2 font-newsreader text-12px ease-in-out duration-300 desktop:p-0 desktop:m-0 desktop:hover:text-dark-brown",
                  {
                    hidden: !isFirstPost,
                    "inline-block tablet:hidden desktop:block": isFirstPost,
                  }
                )}
              >
                Read More
              </Link>
            </div>
          </article>
        );
      })}
      <div
        className={classNames(
          "hidden gap-[50px] flex-col justify-center items-center row-span-2",
          {
            "desktop:flex": !isEntertainmentCategory && !isRealEstateCategory,
            "desktop:col-start-3": isLatestNews,
          }
        )}
      >
        <p className="w-[250px] flex justify-center items-center bg-beige h-full">
          Ad space
        </p>
        <p
          className={classNames(
            "w-[250px] flex justify-center items-center bg-beige h-full",
            {
              hidden: !isLatestNews && !isSportCategory,
            }
          )}
        >
          Ad space
        </p>
      </div>
    </div>
  );
};

export default HomePageCategoryLatestGrid;
